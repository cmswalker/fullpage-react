...
class Fullpage extends React.Component {
  constructor(props) {
    ...
    let slideChildren = this.props.children.filter((c) => c.type.name == 'Slide');
    this.state = {
      name: 'Fullpage',
      slides: [],
      ...
      touchStart: 0,
      // touchSensitivity: this.props.sensitivity || 250,
      touchSensitivity: this.props.sensitivity || 200,
      scrollPending: false
    };
  }
  ...
  onTouchEnd(e) {
    ...
    if (this.state.touchStart > touchEnd - this.state.touchSensitivity) {

      if (this.state.activeSlide == this.state.slidesCount - 1) {//down going down
        return;
      }

      this.scrollToSlide(this.state.activeSlide + 1);

    } else if (this.state.touchStart < touchEnd + Math.abs(this.state.touchSensitivity)) {

      if (this.state.activeSlide == 0) {//up going up
        return;
      }
