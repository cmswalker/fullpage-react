...
class SliderSchema extends React.Component {
  ...
  componentDidMount() {
    ...
    //init
    var winProp = this.props.horizontal ? 'innerWidth' : 'innerHeight';
    resize.call(this, winProp);
  }
  ...
  shouldComponentUpdate(nP, nS) {
    ...
    return (p.slideCount !== nP.slideCount) || (s.slides.length !== nP.slideCount) || (s.height !== nS.height);
  }
  ...
  onTouchEnd(e) {
    ...
    if ((!touchStart) || touchStart > (touchEnd + Math.abs(sensitivity / 2)) ) {
      ...
      return scrollToSlide.call(this, this.node, s.activeSlide + 1);
    }
    ...
    scrollToSlide.call(this, this.node, s.activeSlide - 1);
  }

  onArrowClick() {
    scrollToSlide.call(this, this.node, this.state.activeSlide + 1);
  }
  ...
  onScroll(e) {
    ...
    scrollDirection.scrollFulfilled((fulfilled, direction) => {
      ...
      //NOTE: IS WORKING ON TOP HORI SLIDE
      this.setState({'lastActive': scrollDown ? activeSlide - 1 : activeSlide + 1});
      scrollToSlide.call(this, node, activeSlide, SCROLL_DIRECTION);
    });
  }
  ...
  render() {
    var p = this.props;
    var s = this.state;
    var style = p.styles || {height: '100%'};

    return (
      <div ref={node => this.node = s.node || node} className={renderUtils.defaultClass.call(this)} style={style}>
        {p.children}
      </div>
    )
