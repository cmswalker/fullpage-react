...
class Fullpage extends React.Component {
  ...
  componentDidMount() {
    ...
    fullPageNode.addEventListener('touchmove', this.onTouchStart);
  ...
  componentWillUnmount() {
    ...
    fullPageNode.removeEventListener('touchmove', this.onTouchStart);
  ...
  scrollToSlide(slide, override) {
    var fullpageNode = this.node;

    if (override) {
      return scrollTo.call(this, fullpageNode, SCROLL_DIR, this.state.slides[slide], 100, () => {
    ...
    scrollTo(fullpageNode, SCROLL_DIR, this.state.slides[slide], 600, () => {
      this.setState({'activeSlide': slide});
      this.setState({'scrollPending': false});
    });
  }
  ...
  onTouchEnd(e) {
    ...
    const sensitivity = this.state.touchSensitivity;
...
//
//   scrollTo(fullpageNode, SCROLL_DIR, comp.state.slides[activeSlide], 500, () => {
