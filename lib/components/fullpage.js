...
class Fullpage extends React.Component {
  ...
  componentDidMount() {
    let fullPageNode = ReactDOM.findDOMNode(this);
  ...
  componentWillUnmount() {
    ...
    fullPageNode.removeEventListener('wheel', this.onScroll);
    fullPageNode.removeEventListener('touchstart', this.onTouchStart);
    fullPageNode.removeEventListener('touchend', this.onTouchEnd);
    fullPageNode.removeEventListener('keydown', this.checkKey);
  ...
  checkKey(e) {
    let direction = null;
    e = e || window.event;
    if (CONSTS.KEY_IDX[e.keyCode]) {
      direction = CONSTS.KEY_IDX[e.keyCode];
    } else {
      return false;
    }
  ...
  scrollToSlide(slide, override) {
    if (override) {
      return scrollTo.call(this, getBody(), SCROLL_DIR, this.state.slides[slide], 100, () => {
    ...
    scrollTo(getBody(), SCROLL_DIR, self.state.slides[slide], 600, function() {
  ...
  render() {
    return (
      <div className={renderUtils.defaultClass.call(this)} style={{height: this.state.height, width: '100%'}}>
...
function handleScroll(e, comp, axis) {
  ...
  scrollTo(getBody(), SCROLL_DIR, comp.state.slides[activeSlide], 500, () => {
