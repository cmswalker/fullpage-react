...
const Fullpage = ({ window, document, children, slides, isNext, activeSlide = 0}) => {
  ...
  return (
    <div className='Fullpage'>
      {children}
      {slides.map((sl, i) => {
        const slProps = sl.props || {};

        const isActive = i === activeSlide;
        const nextQualififier = isNext[i];

        if (!isActive && !nextQualififier) {
          return null;
        }

        const key = `vertical-slide-${i}`;
        const slideProps = { ...slProps, window, document, key, isActive, isNext: nextQualififier };
        const slideChildren = sl.children;

        if (sl.type.name === 'HorizontalSlider') {
          const { name } = sl;
          const horizontalSliderProps = { ...slideProps, name }
          return <HorizontalSlider {...horizontalSliderProps}/>;
        }

        return <Slide {...slideProps}>{slideChildren}</Slide>
      })}
    </div>
  )
}
...
class FullpageEmitter extends React.Component {
  ...
  componentDidMount() {
    ...
    const { window, node, scrollSensitivity, touchSensitivity } = this;

    window.addEventListener('resize', this.resize.bind(this, window));
    const ss = new ScrollSwipe({
      target: node,
      scrollSensitivity: scrollSensitivity * 3,
      touchSensitivity: touchSensitivity * 3,
      scrollPreventDefault: true,
      touchPreventDefault: true,
  ...
  onScrollAction({ direction, intent, startEvent }) {
    ...
    // at this point we are dedicating to a vertical scroll
    if (direction === 'VERTICAL') {
      return this.onVerticalScroll(dir[intent]);
    }
    ...
    const horizonalNode = path.find(p => {
      if (!p.dataset) {
        return false;
      }
    ...
    this.onHorizontalScroll(dir[intent], horizonalNode);
  }

  onVerticalScroll(intent) {
    const { state, window, ss, verticalRoot, name } = this;
    const { activeSlide } = state;
    const { slides } = this.props;
  ...
  onHorizontalScroll(intent, horizonalNode) {
    console.log('onHori', intent);
  }

  finishScroll({ domRoot, props, state, newState }) {
    this.onSlideChange(domRoot, props, state, newState);
  ...
  handleScroll(node, winProp, to, newState, domRoot) {
    const { ss, props, state, scrollSpeed } = this;
    ss.flush();

    scrollTo(node, winProp, to, scrollSpeed, () => {
      newState.scrollPending = false;
      this.setState(newState, () => {
        this.finishScroll({ domRoot, props, state, newState });
      });
    });
  }
  ...
  render() {
    ...
    const backwardsEligible = props.infinite || state.activeSlide > 0;
    const forwardEligible = props.infinite || state.activeSlide < (props.slides.length - 1);

    const isNext = {};
    if (backwardsEligible) {
      isNext[state.activeSlide - 1] = true;
    }
    ...
    const fullPageProps = { isNext, ...props, ...state };
